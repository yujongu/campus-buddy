...
export default class NotificationScreen extends Component{
    ...
    const subscriber = onSnapshot(doc(db, "requests", auth.currentUser?.email), (doc) => {
      doc.data()['from_request'].map((request) => {
        ...
        if(sp[sp.length - 1] == "friend"){
          this.setState({friend_requests: [...this.state.friend_requests, request]})
  ...
  renderItem = (item) => {
    ...
    if(type == "friend"){
      ...
      return(
      <TouchableOpacity style={styles.item} onPress={() => Alert.alert("Information", str)}>
        <Text numberOfLines = {1} style={{color: 'black', fontSize: 15}}>{str}</Text>
      ...
      return (
        <TouchableOpacity style = {styles.item2} onPress={() => Alert.alert("Information", info)}>
